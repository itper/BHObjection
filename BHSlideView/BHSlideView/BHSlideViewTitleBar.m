...
#import "GJGCTitleBar.h"

#define GJGC_TITLE_BAR_BASE_TAG 10100
#define GJGC_TITLE_BAR_PADDING_BOTTOM 1
#define RGB(R, G, B, A) \
[UIColor colorWithRed:R/255.f green:G/255.f blue:B/255.f alpha:A]

@interface ListenerWrap:NSObject
@property (nonatomic,weak)id<GJGCTitleBarOnClickListener>listener;
-(instancetype)initWithWeakObj:(id<GJGCTitleBarOnClickListener>)obj;
@end
...
-(void)reload{
    [self.scrollView.subviews enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        ...
        if(view.tag>=GJGC_TITLE_BAR_BASE_TAG)[view removeFromSuperview];
    }];
    [self loadViews];
    self.selectedIndex = 0;
    
}
-(void)setDelegate:(id<GJGCTitleBarDelegate>)delegate{
    _delegate = delegate;
    [self reload];
}
//可注册多个侦听
...
-(void)createView{
    ...
    self.scrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, self.frame.size.height-GJGC_TITLE_BAR_PADDING_BOTTOM)];
...
-(void)loadViews{
    ...
    for (NSInteger i = 0; i<[self.delegate itemCount:self]; i++) {
        ...
        button.frame = rect;
        button.tag = GJGC_TITLE_BAR_BASE_TAG+i;
...
-(void)click:(UIButton*)button{
    [self setSelectedIndex:button.tag-GJGC_TITLE_BAR_BASE_TAG];
    for (NSObject *obj in self.listeners) {
        [((ListenerWrap*)obj).listener onClick:button.tag-GJGC_TITLE_BAR_BASE_TAG];
    }
}
-(UIButton *)selectedButton{
    return (UIButton*)[self.scrollView viewWithTag:self.selectedIndex+GJGC_TITLE_BAR_BASE_TAG];
}
-(UIButton *)previousButton{
    return (UIButton*)[self.scrollView viewWithTag:self.previousIndex+GJGC_TITLE_BAR_BASE_TAG];
}
...
//    NSDate *date = [NSDate new];
    ...
    UIButton *targetButton = (UIButton*)[self.scrollView viewWithTag:currentPage+GJGC_TITLE_BAR_BASE_TAG+1];
    UIButton *currentButton = (UIButton*)[self.scrollView viewWithTag:currentPage+GJGC_TITLE_BAR_BASE_TAG];
