...
#import "GJGCSlidePageView.h"
@interface GJGCSlidePageView ()<UIScrollViewDelegate,GJGCTitleBarOnClickListener>
...
@implementation GJGCSlidePageView
...
-(void)createView:(CGFloat)titleHeight{
    ...
    self.titleBar = [[GJGCTitleBar alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, titleHeight)];
...
-(void)setDelegate:(id<GJGCSlidePageDelegate>)delegate{
    _delegate = delegate;
    [self loadData];
    [self setSelectedIndex:0];
}
-(void)setTitleBarDelegate:(id<GJGCTitleBarDelegate>)titleBarDelegate{
